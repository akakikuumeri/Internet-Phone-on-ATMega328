/*
 IP phone on ATMEGA328
 
 Simulataneously capture and send as well as receive and play back audio from a XBee serial.
...
int BufferHead=0;
int BufferTail=0;
unsigned char micdata[BUFFER_SIZE];
int micbufHead=0;
int micbufTail=0;
...
ISR(TIMER1_COMPA_vect)
{
    ...
    micdata[micbufHead] = (unsigned int)(analogRead(A0)/4);//gets converted to one byte. Could be optimized by ditching the orebuilt function and just reading one register.
    micbufHead = (micbufHead+1)%BUFFER_SIZE;
    micdatasize++;
}//End Interrupt
void startPlayback()
{
...
void loop()
{
    ...
    if (micdatasize > 128) //if we have some audio recorded. by Akaki
    {
      //Serial.println("Hello");
      while (micdatasize>0) //go until all is sent out
      {
        Serial.write(micdata[micbufTail]);//you could join it all in one string, but im not sure how the interrupts would like it if you were sending long serials.
        micdatasize--;
